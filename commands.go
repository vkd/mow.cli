...
import (
	...
	"github.com/vkd/mow.cli/internal/parser"
)
...
func (c *Cmd) Bool(p BoolParam) *bool {
	...
	switch x := p.(type) {
	case BoolOpt:
		c.mkOpt(container.Container{Name: x.Name, Desc: x.Desc, HideDefault: x.HideDefault, Value: value, ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	case BoolArg:
		c.mkArg(container.Container{Name: x.Name, Desc: x.Desc, HideDefault: x.HideDefault, Value: value, ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	default:
...
func (c *Cmd) Var(p VarParam) {
	switch x := p.(type) {
	case VarOpt:
		c.mkOpt(container.Container{Name: x.Name, Desc: x.Desc, HideDefault: x.HideDefault, Value: p.value(), ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	case VarArg:
		c.mkArg(container.Container{Name: x.Name, Desc: x.Desc, HideDefault: x.HideDefault, Value: p.value(), ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	default:
		panic(fmt.Sprintf("Unhandled param %v", p))
	}
}
...
func (c *Cmd) printHelp(longDesc bool) {
	...
	if len(c.args) > 0 {
		...
		for _, arg := range c.args {
			var (
				value = formatValueForHelp(arg.HideDefault, arg.DefaultValue)
			)
			printTabbedRow(w, arg.Name, joinStrings(arg.Desc, value))
		}
	}
	...
	if len(c.options) > 0 {
		...
		for _, opt := range c.options {
			...
			var (
				...
				value    = formatValueForHelp(opt.HideDefault, opt.DefaultValue)
			)
			printTabbedRow(w, optNames, joinStrings(opt.Desc, value))
		}
	}

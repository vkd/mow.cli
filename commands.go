...
func (c *Cmd) Bool(p BoolParam) *bool {
	...
	switch x := p.(type) {
	case BoolOpt:
		c.mkOpt(container.Container{Name: x.Name, Desc: x.Desc, HideValue: x.HideValue, Value: value, ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	case BoolArg:
		c.mkArg(container.Container{Name: x.Name, Desc: x.Desc, HideValue: x.HideValue, Value: value, ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	default:
...
func (c *Cmd) Var(p VarParam) {
	switch x := p.(type) {
	case VarOpt:
		c.mkOpt(container.Container{Name: x.Name, Desc: x.Desc, HideValue: x.HideValue, Value: p.value(), ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	case VarArg:
		c.mkArg(container.Container{Name: x.Name, Desc: x.Desc, HideValue: x.HideValue, Value: p.value(), ValueSetByUser: x.SetByUser, Hidden: x.Hidden})
	default:
		panic(fmt.Sprintf("Unhandled param %v", p))
	}
}
